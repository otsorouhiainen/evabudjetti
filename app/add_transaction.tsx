...
export default function AddTransaction() {
  ...
  const isValid = useMemo(() => name.trim().length > 0 && !!date && !!amount, [name, amount, date]);
    ...
    <>
      ...
      {/* Top header */}
            <View style={styles.headerRow}>
              <Text category="h5" style={styles.headerTitle}>Lisää uusi</Text>
            </View>
            ...
            <View style={styles.segmentWrap}>
              {(['TULO', 'MENO'] as const).map((opt) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.segmentItem,
                    opt === type && { backgroundColor: theme['color-primary-500'] },
                  ]}
                >
                  <Text
                    category="s1"
                    style={[
                      styles.segmentText,
                      opt === type ? { color: '#fff' } : { color: theme['color-primary-500'] },
                    ]}
                  >
                    {opt}
                  </Text>
            ...
            {/* Category chips */}
            <Text appearance="hint" style={styles.labelTop}>Kategoria</Text>
            <View style={styles.chipsRow}>
              {(['Tuki', 'Vuokratulo', 'Muu', 'Lisää kategoria (+)'] as CategoryKey[]).map((c) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      selected && { backgroundColor: theme['color-primary-600'] },
                    ]}
                  >
            ...
            <Card disabled style={styles.formCard}>
              <Text style={styles.inputLabel}>{type === 'TULO' ? 'TULON nimi' : 'MENON nimi'}</Text>
              <Input
                value={name}
                onChangeText={setName}
                placeholder={type === 'TULO' ? 'Tulon nimi' : 'Menon nimi'}
                style={styles.input}
                size="medium"
              />
              ...
              <Input
                ...
                placeholder="000,00 €"
                accessoryRight={() => (
                  <Text appearance="hint" style={{ marginRight: 6 }}>€</Text>
                )}
                style={styles.input}
                size="medium"
              />
              ...
              {Platform.OS === 'web' ? (
                <Input
                  ...
                  onChangeText={val => setDate(val ? new Date(val) : null)}
            ...
            <Button
              size="large"
              disabled={!isValid}
              style={[styles.submitBtn, { backgroundColor: theme['color-primary-500'] }]}
              onPress={() => {
                // submit handler placeholder
                console.log({ type, category, name, amount, date, repeat });
              }}
            >

...
const POSITIVE_TX: Txn[] = [...FUTURE_TX, ...PAST_TX].filter(
	(ex) => ex.amount >= 0,
);
const NEGATIVE_TX: Txn[] = [...FUTURE_TX, ...PAST_TX].filter(
	(ex) => ex.amount < 0,
);
					...
					<Layout style={styles.screen} level="1">
						{/* Top header */}
							...
							<View style={styles.segmentWrap}>
								{(
									['PÄIVÄ', 'KUUKAUSI', 'VUOSI'] as Period[]
								).map((p) => {
